...
void TestCellReferences() {
	...
	auto ps = sheet->GetCell("A2"_pos)->GetReferencedCells();
...
/*void TestOwnFormulas() {
	auto sheet = CreateSheet();
	auto evaluate = [&](std::string expr) {
		return std::get<double>(ParseFormula(std::move(expr))->Evaluate(*sheet));
	};
	sheet->SetCell("A1"_pos, "1");
	sheet->SetCell("A5"_pos, "2");
	sheet->SetCell("B3"_pos, "=A12+A5+12");
	//sheet->ClearCell("A12"_pos);
	ASSERT_EQUAL(evaluate("A1+B3*2"), 29);
