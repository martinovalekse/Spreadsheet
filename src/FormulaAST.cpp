...
public:
	...
	virtual double Evaluate(const SheetInterface& sheet, std::function<void()> lambd) const = 0;
...
public:
	...
	double Evaluate(const SheetInterface& sheet, std::function<void()> lambd) const override {
		ExprPrecedence operation(GetPrecedence());
		double lhs(lhs_->Evaluate(sheet, lambd));
		double rhs(rhs_->Evaluate(sheet, lambd));
		double result = 0;
		if (operation == ExprPrecedence::EP_ADD) {
			if (std::abs( rhs ) > std::numeric_limits<double>::max() - std::abs( lhs ) ) {
				throw FormulaError(FormulaError::Category::Div0);
			}
			result = lhs + rhs;
		} else if (operation == ExprPrecedence::EP_SUB) {
			if (std::abs( rhs ) > std::numeric_limits<double>::max() - std::abs(lhs ) ) {
				throw FormulaError(FormulaError::Category::Div0);
			}
			result = lhs - rhs;
		} else if (operation == ExprPrecedence::EP_MUL) {
			if (std::abs( rhs ) > std::numeric_limits<double>::max() / std::abs( lhs ) ) {
				throw FormulaError(FormulaError::Category::Div0);
			}
			result = lhs * rhs;
		} else if (operation == ExprPrecedence::EP_DIV) {
			if ((rhs == 0 || !std::isfinite( rhs )) )  {
				throw FormulaError(FormulaError::Category::Div0);
			}
...
public:
	...
	double Evaluate(const SheetInterface& sheet, std::function<void()> lambd) const override {
		double result = operand_->Evaluate(sheet, lambd);
...
public:
	...
	double Evaluate(const SheetInterface& sheet, std::function<void()> /*lambd*/) const override {
		//lambd();
		double result = 0.0;
		auto ptr = sheet.GetCell(*cell_);
		if (ptr != nullptr) {
				auto item = ptr->GetValue();
			if (std::holds_alternative<double>(item)) {
				result = std::get<double>(item);
			}
			if (std::holds_alternative<std::string>(item)) {
				 if (std::get<std::string>(item).empty()) {
					 result = 0.0;
				 } else {
					if (std::get<std::string>(item).find_first_not_of("0123456789") == std::string::npos) {
						result = std::stoi(std::get<std::string>(item));
					} else {
						throw FormulaError(FormulaError::Category::Value);
					}
				}
			}
			if (std::holds_alternative<FormulaError>(item)) {
				throw std::get<FormulaError>(item);
			}
		}
		return result;
	}
...
public:
	...
	double Evaluate(const SheetInterface& /*sheet*/, std::function<void()> /*lambd*/) const override { // заглушка
		return value_;
	}
...
double FormulaAST::Execute(const SheetInterface& sheet,  std::function<void()> lambd) const {
	return root_expr_->Evaluate(sheet, lambd);
}
